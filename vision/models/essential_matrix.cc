...
void EssentialMatrix::GetRotationAndTranslation(double rotation[3][3],
  ...
  // Choose the two rows that produce the largest cross product magnitude.
  Vector3d large_a = essential_mat_.row(0);
  Vector3d large_b = essential_mat_.row(1);
  if (large_a.cross(large_b).norm() <
      essential_mat_.row(0).cross(essential_mat_.row(2)).norm())
    large_b = essential_mat_.row(2);

  if (large_a.cross(large_b).norm() <
      essential_mat_.row(1).cross(essential_mat_.row(2)).norm()) {
  ...
  std::cout << "my u = \n" << u << std::endl;
  std::cout << "my v = \n" << v << std::endl;
  std::cout << "r = " << u*d*(v.transpose()) << std::endl;
  t = clock() - t;
